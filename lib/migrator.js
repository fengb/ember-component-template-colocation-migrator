...
module.exports = class Migrator {
  ...
  findComponentClasses() {
    return (
      glob
        .sync(`{${this.appBackingClassesDir},${this.addonBackingClassesDir}}/**/*.{js,ts}`)
        // AST parser breaks on type definition files
  ...
  /**
   * @param {string[]} newTemplateFilePaths
   */
  wrapDivAttributesForTemplateOnly(newTemplateFilePaths) {
    for (const templatePath of newTemplateFilePaths) {
      if (
        fs.existsSync(templatePath.replace(/.hbs$/, '.js')) ||
        fs.existsSync(templatePath.replace(/.hbs$/, '.ts'))
      ) {
        // Found a corresponding js/ts file -- not wrapping
        continue;
      }

      const contents = fs.readFileSync(templatePath, { encoding: 'utf8' });
      if (contents.includes('...attributes')) {
        // Already uses ...attributes -- not wrapping
        continue;
      }
...
${contents.replace(/^(.)/mg, '  $1')}
  ...
  async execute() {
    ...
    // Buggy -- no partials need to be accounted for
    ...
    let newTemplateFilePaths;
    if (this.structure === 'flat') {
      newTemplateFilePaths = this.changeComponentStructureToFlat(templateFilePaths);
    } else if (this.structure === 'nested') {
      newTemplateFilePaths = this.changeComponentStructureToNested(templateFilePaths);
    }

    this.wrapDivAttributesForTemplateOnly(newTemplateFilePaths);
